...
def main(args):
  ...
  cmd = '%s -M %s -W %s -w %s bash -c "source ~/.bashrc; %s %s -f %s" > %s 2>> %s' % (args.runsolver, args.memlimit, args.timelimit, args.watcher, args.solver, ''.join(args.solver_args), ' '.join(args.filename), args.stdout, args.stderr)
