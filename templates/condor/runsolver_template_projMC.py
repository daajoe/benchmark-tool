...
def main(args):
  ...
  cmd = '%s -M %s -W %s -w %s %s %s %s -fpv=%s > %s 2>> %s' % (args.runsolver, args.memlimit, args.timelimit, args.watcher, args.solver, ''.join(args.solver_args), ' '.join(args.filename), args.stdoutprepare, args.stdout, args.stderr)
  sys.stderr.write('COMMAND=%s\n' %cmd)
