...
def main(args):
  ...
  cmd = '%s -M %s -W %s -w %s %s %s %s > %s 2>> %s' % (args.runsolver, args.memlimit, args.timelimit, args.watcher, args.solver, args.solver_args, ' '.join(args.filename), args.stdout, args.stderr)
