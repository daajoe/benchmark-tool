...
def main(args):
    ...
    cmd = '%s -M %s -W %s -w %s %s %s -t %s --runid %s -f %s > %s 2>> %s' % (
        args.runsolver, args.memlimit, args.timelimit, args.watcher, args.solver, ''.join(args.solver_args), tmp,
        args.run,
        ' '.join(args.filename), args.stdout, args.stderr)
    ...
    es.index(index=myindex, doc_type="test-type", id=42,
             body={"any": "data", "timestamp": datetime.now()})

    print es.get(index=myindex, doc_type="test-type", id=42)['_source']
