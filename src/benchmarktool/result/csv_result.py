...
class InstanceTable(ResultTable):
    ...
    def output_cactus_plot(self, plots, filename, benchmark, mapping, indices=('time',), ignore_vbest=False,
                           limit=5):
    ...
    def printSheet(self, prefix, project_name, suffix, results, keys):
        ...
        df_time_range['time_range'] = pd.cut(df_time_range['time'], np.arange(0, 1800, 120))
        ...
        df = df.sort_values([u'benchmark_name', u'class', u'time']).groupby([u'instance'],as_index=False).first()
        # exit(1)
        #select frist value
        ...
        for k in output.iterkeys():
            with open(os.path.join(prefix, '%s-%s%s' % (project_name, k, suffix)), 'w') as outfile:
                ...
                if k == 'by-run':
                    ...
                    output[k].sort_values(by=[('instance', ''), ('time', 'mean'), ], inplace=True)
                elif k == 'by-class':
                    output[k].sort_values(
                        by=['benchmark_name', 'class', ('time', 'mean'), ],
                        ascending=[True, True, True], inplace=True)
                    output[k].reset_index()
                elif k == 'by-time':
                    output[k].sort_values(
                        by=['benchmark_name', 'time_range', ('time', 'mean'), ],
                        ascending=[True, True, False], inplace=True)
                    output[k].reset_index()
                elif k == 'by-mem':
                    output[k].sort_values(
                        by=['benchmark_name', 'mem_range', ('time', 'mean'), ],
                        ascending=[True, True, True], inplace=True)
                    output[k].reset_index()
                elif k == 'by-benchmark':
                    output[k].sort_values(
                        by=['benchmark_name', ('instance', 'count'), ('time', 'mean'), ],
                        ascending=[True, False, True], inplace=True)
                elif k == 'by-all':
                    output[k].sort_values(
                        by=[('instance', 'count'), ('time', 'mean'), ],
    ...
    def compute_short_df(self, df, vbest):
        # take short vbest
        vbest_short = vbest[['instance', 'solver_config', 'time']]
        # ignore warning deliberately
        pd.options.mode.chained_assignment = None
        vbest_short['solver_config'] = 'vbest'
        pd.options.mode.chained_assignment = 'warn'
        short_df = df[['instance', 'solver_config', 'time']]
        short_df = short_df.append(vbest_short)
        short_df = short_df.reindex_axis(['solver_config', 'instance', 'time'], axis=1)
        return short_df
    ...
    def printSheetTrellis(self, prefix, project_name, suffix, results, keys):
        ...
        output['by-instance'] = df.groupby(['instance', 'benchmark_name', 'class', 'solver_config', 'solver',
            ...
            {'run': 'count', 'width': [np.mean, np.max, np.min, np.std],
             ...
             'time': [np.mean, np.max, np.min, np.std], 'time': [np.mean, np.max, np.min, np.std],
             'error': [np.max]})
        ...
        output['by-class'] = df.groupby(
            ...
            {'instance': 'count', 'width': [np.mean, np.max, np.min, np.std],
             'ubound': [np.mean, np.max, np.min, np.std], 'abs_improvement': [np.max, np.count_nonzero, np.sum],
             'time': [np.mean, np.max, np.min, np.std], 'time': [np.mean, np.max, np.min, np.std]}).reset_index()
        ...
        output['by-width'] = df_width.groupby(
            ...
            {'instance': 'count', 'width': [np.mean, np.max, np.min, np.std],
             'ubound': [np.mean, np.max, np.min, np.std], 'abs_improvement': [np.max, np.count_nonzero, np.sum],
             'time': [np.mean, np.max, np.min, np.std], 'time': [np.mean, np.max, np.min, np.std, np.sum],
             'solved': np.sum}).reset_index()
