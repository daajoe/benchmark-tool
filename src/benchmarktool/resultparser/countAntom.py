...
countAntom_re = {
    "#models": ("long", re.compile(r"^\s*c\s*model\s*count\.+:\s*(?P<val>\d+)\s*$")),
    "#threads": ("long", re.compile(r"^\s*c\s*number of threads\.+:\s*(?P<val>\d+)\s*$")),
    "memorySize": ("long", re.compile(r"^\s*c\s*memory size\.+:\s*(?P<val>\d+)\s*$")),
    "#backtracks": ("long", re.compile(r"^\s*c\s*TOTAL backtracks\.+:\s*(?P<val>\d+)\s*$")),
    "#decisions": ("long", re.compile(r"^\s*c\s*#decisions\.+:\s*(?P<val>\d+)\s*$")),
    "#bcpOperations": ("long", re.compile(r"^\s*c\s*bcp operations\.+:\s*(?P<val>\d+)\s*$"))
}


def countAntom(root, runspec, instance):
    """
    ...
    if instance_str == 'mod2-rand3bip-unsat-120-3.shuffled-as.sat05-2626.cnf':
        asdf = 0
    ...
    if finished:
        ...
        for line in content:
            for val, reg in countAntom_re.items():
                m = reg[1].match(line)
