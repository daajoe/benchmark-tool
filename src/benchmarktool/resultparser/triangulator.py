...
projmc_re = {
    "objective": (
    "float", re.compile(r"^Fractional hypertreewidth: (?P<val>[0-9]+(\.[0-9]+)?)"), lambda x: x),
    #"objective_presolved": (
...
empty_pmc = set() # read_empty_pmc()

def triangulator(root, runspec, instance):
    """
    ...
    try:
        ...
        for line in err_content.splitlines():
            ...
            for val, reg in projmc_re.items():
                m = reg[1].match(line)
                if m and reg[0] == "objective" and foundprev == False:
                    continue
                if m and (res[val][1] == -1 or res[val][1] == 'nan'): res[val] = (reg[0], reg[2](m.group("val")))
            foundprev = line.startswith("# solutions")
            lines += 1
    ...
    #if lines <= 2 and instance.instance not in empty_pmc:
    #    res['solved'] = ('int', 1)
    #    res['time'] = ('float', 0.1)
    #    res['timeout'] = ('int', 0)
    ...
    try:
        ...
        if res['objective'][1] != 'nan':
            res['solved'] = ('int', 1)
        if res['width'][1] == -1:
            res['width'] = ('int', inst_info_dict[instance.instance]['width'])
