...
sharpsat_re = {
    "#models": ("long", re.compile(r"^\s*(?P<val>\d+)\s*$"))
}


def sharpsat(root, runspec, instance):
    """
    ...
    if finished:
        ...
        for line in content:
            for val, reg in sharpsat_re.items():
                m = reg[1].match(line)
        ...
        elif "Assertion `(data_size >> bits_of_data_size()) == 0' failed." in errorFile:
            res["timeout"] = ("float", 1)
