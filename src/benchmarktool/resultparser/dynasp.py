...
dynasp_re = {
    "#models": ("long", re.compile(r"^\s*SOLUTION COUNT:\s*(?P<val>\d*)\@\d*,\d*\s*$")),
    "width": ("long", re.compile(r"^\s*TREEWIDTH:\s*(?P<val>\d*)\s*$")),
    "time_htd": ("float", re.compile(r"^.*,.*,.*,(?P<val>[0-9]+(\.[0-9]+)?),tree decomposition time\s*$"))
}
