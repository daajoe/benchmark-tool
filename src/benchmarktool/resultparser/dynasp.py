...
dynasp_re = {
    "#models": ("float", re.compile(r"^\s*solution\s*count\s*:\s*(?P<val>\d*\.\d+|\d+|\d*\.\d+e[-+]\d+)\s*$")),
    "#rules": ("float", re.compile(r"^\s*Static\s*Rules\s*:\s*(?P<val>\d*\.\d+|\d+|\d*\.\d+e[-+]\d+)\s*$")),
    "#constraints": ("float", re.compile(r"^\s*Static\s*Constraints\s*:\s*(?P<val>\d*\.\d+|\d+|\d*\.\d+e[-+]\d+)\s*$")),
    "#leaf": ("float", re.compile(r"^\s*LEAF\s*:\s*(?P<val>\d*\.\d+|\d+|\d*\.\d+e[-+]\d+)\s*$")),
    "#exchange": ("float", re.compile(r"^\s*EXCHANGE\s*:\s*(?P<val>\d*\.\d+|\d+|\d*\.\d+e[-+]\d+)\s*$")),
    "#join": ("float", re.compile(r"^\s*JOIN\s*:\s*(?P<val>\d*\.\d+|\d+|\d*\.\d+e[-+]\d+)\s*$")),
    "#firsts": ("float", re.compile(r"^\s*FIRSTS\s*:\s*(?P<val>\d*\.\d+|\d+|\d*\.\d+e[-+]\d+)\s*$")),
    "#reuses": ("float", re.compile(r"^\s*REUSES\s*:\s*(?P<val>\d*\.\d+|\d+|\d*\.\d+e[-+]\d+)\s*$")),
    "#splitups": ("float", re.compile(r"^\s*SPLITUPS\s*:\s*(?P<val>\d*\.\d+|\d+|\d*\.\d+e[-+]\d+)\s*$")),
    "#treewidth": ("float", re.compile(r"^\s*treewidth\s*:\s*(?P<val>\d*\.\d+|\d+|\d*\.\d+e[-+]\d+)\s*$"))
}

def dynasp(root, runspec, instance):
    """
    Extracts some sat statistics.
    """
    #TODO: merge with regular dynasp 
    res = {
