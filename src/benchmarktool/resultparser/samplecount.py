...
samplecount_re = {
    "seed": ("float", re.compile(r"^\s*random\sseed\sused:\s(?P<val>[-+]?[0-9]*\.?[0-9]+)$")),
    "last_random": ("float", re.compile(r"^\s*last\srandom\snumber\sgenerated:\s(?P<val>[-+]?[0-9]*\.?[0-9]+)$")),
    "#models": ("float", re.compile(r"^\s*total\sestimate:\s(?P<val>[-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+))$"))
}


def samplecount(root, runspec, instance):
    """
    ...
    if finished:
        ...
        for f in [instance.instance + ".txt", instance.instance + ".solver", "runsolver.solver"]:
            ...
            for line in codecs.open(os.path.join(root, f), errors='ignore', encoding='utf-8'):
                ...
                for val, reg in samplecount_re.items():
                    m = reg[1].match(line)
