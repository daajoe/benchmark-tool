...
sharpCDCL_re = {
    "#models": ("long", re.compile(r"^\s*Found Models:\s*(?P<val>\d*\.\d+|\d+|\d*\.\d+e[-+]\d+)\s*$")),
    "indet": ("long", re.compile(r"^\s*(?P<val>INDETERMINATE)\s*$"))
}


def sharpCDCL(root, runspec, instance):
    """
    ...
    if finished:
        # errors='ignore',
        content = codecs.open(os.path.join(root, '%s.err' % instance_str), encoding='utf-8')

        errorFile = open(os.path.join(root, instance.instance + ".err"),"r").read()

        for line in content:
            for val, reg in sharpCDCL_re.items():
                m = reg[1].match(line)
