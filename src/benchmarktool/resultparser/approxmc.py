...
approxmc_re = {
    "#models": ("long", re.compile(r"^\s*With confidence at least:\d*\.\d*\s*Approximate count is :\s*(?P<val>\d*x\d*\^\d*)\s*$"))
}


def approxmc(root, runspec, instance):
    """
    ...
    if finished:
        ...
        for line in content:
            for val, reg in approxmc_re.items():
                m = reg[1].match(line)
        ...
        elif " Assertion `decisionLevel() == 0' failed" in errorFile:
            res["timeout"] = ("float", 1)
            res["time"] = ('float', runspec.project.job.timeout + 7)
        elif "Too long clause!" in errorFile:
            res["timeout"] = ("float", 1)
            res["time"] = ('float', runspec.project.job.timeout + 8)
