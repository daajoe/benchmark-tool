...
lingeling_re = {
    "#vars": ("int", re.compile(r"^.* (?P<val>\d*) variables.*$")),
    "#lit": ("int", re.compile(r"^.* (?P<val>\d*) literals.*$")),
    "#cls": ("int", re.compile(r"^.* (?P<val>\d*) clauses.*$")),
    "#desicions": ("float", re.compile(r"^.* (?P<val>\d*) decisions.*$")),
    "#conflicts": ("float", re.compile(r"^.* (?P<val>\d*) conflicts.*$"))
}


def lingeling(root, runspec, instance):
    """
    ...
    if finished:
        ...
        for line in content:
            for val, reg in lingeling_re.items():
                m = reg[1].match(line)
        ...
        elif "std::bad_alloc" in content or "Maximum VSize exceeded: sending SIGTERM then SIGKILL" in f:
            res["timeout"] = ("float", 1)
            res["time"] = ('float', runspec.project.job.timeout + 4)
        elif "inf" in content:
            res["timeout"] = ("float", 1)
