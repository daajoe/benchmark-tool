...
dsharp_re = {
    "#models": ("long", re.compile(r"^\s*# of solutions:\s*(?P<val>\d*\.\d+|\d+|\d*\.\d+e[-+]\d+)\s*$")),
    "#clauses": ("float", re.compile(r"^\s*#Clauses:\s*(?P<val>\d*\.\d+|\d+|\d*\.\d+e[-+]\d+)\s*$")),
    "#var": ("float", re.compile(r"^\s*#Vars:\s*(?P<val>\d*\.\d+|\d+|\d*\.\d+e[-+]\d+)\s*$")),
    "#desicions": ("float", re.compile(r"^\s*Num. decisions:\s*(?P<val>\d*\.\d+|\d+|\d*\.\d+e[-+]\d+)\s*$")),
    "infinite": ("string", re.compile(r"^\s*# of solutions:\s*(?P<val>inf)\s*$")),
    "nan": ("string", re.compile(r"^\s*# of solutions:\s*(?P<val>-nan)\s*$"))
}


def dsharp(root, runspec, instance):
    """
    ...
    if finished:
        ...
        for line in content:
            for val, reg in dsharp_re.items():
                m = reg[1].match(line)
        ...
        if res['wall'][1] >= timeout:
            ...
            res["timeout"] = ("float", 1)
            res["time"] = ('float', runspec.project.job.timeout + 4)
        elif "infinite" in res or "nan" in res:
            res["timeout"] = ("float", 1)
