...
runsolver_re = {
    ...
    "abort": ("string", re.compile(r"^\s*Child\s*ended\s*because\s*it\s*received\s*signal\s*6\s*\((?P<val>SIGABRT)\)\s*"), lambda x: x),
...
d4_re = {
    "#models": ("long", re.compile(r"^\s*s\s*(?P<val>\d+)\s*$"))
}


def d4(root, runspec, instance):
    """
