...
function information
{
	echo "CEGARTIX v0.4 (competition version)"
	echo "Wolfgang Dvorak"
	echo "Matti Jaervisalo"
...
function solver
{
	...
	if   [ "$problem" = "DS-PR" ];
	then
		$CEGARTIX $fileinput -semantics=pref -mode=skept -argument=$additional -oracle=$ORACLE
	elif [ "$problem" = "DS-SST" ]; 
	then
		$CEGARTIX $fileinput -semantics=semi -mode=skept -argument=$additional -oracle=$ORACLE
	elif [ "$problem" = "DS-STG" ]; 
	then	
		$CEGARTIX $fileinput -semantics=stage -mode=skept -argument=$additional -oracle=$ORACLE
	elif [ "$problem" = "DC-SST" ]; 
	then
		$CEGARTIX $fileinput -semantics=semi -mode=cred -argument=$additional -oracle=$ORACLE
	elif [ "$problem" = "DC-STG" ]; 
	then	
		$CEGARTIX $fileinput -semantics=stage -mode=cred -argument=$additional -oracle=$ORACLE
	elif [ "$problem" = "SE-PR" ]; 
	then	
		$CEGARTIX $fileinput -semantics=pref -mode=first -argument=$additional -oracle=$ORACLE -depthArg=-1
	elif [ "$problem" = "EE-PR" ]; 
	then	
		$CEGARTIX $fileinput -semantics=pref -mode=enum -argument=$additional -oracle=$ORACLE -depthArg=-1
	elif [ "$problem" = "SE-SST" ]; 
	then	
		$CEGARTIX $fileinput -semantics=semi -mode=first -argument=$additional -oracle=$ORACLE -depthArg=-1
	elif [ "$problem" = "EE-SST" ]; 
	then	
		$CEGARTIX $fileinput -semantics=semi -mode=enum -argument=$additional -oracle=$ORACLE -depthArg=-1
	elif [ "$problem" = "SE-STG" ]; 
	then	
		$CEGARTIX $fileinput -semantics=stage -mode=first -argument=$additional -oracle=$ORACLE -depthArg=-1
	elif [ "$problem" = "EE-STG" ]; 
	then	
		$CEGARTIX $fileinput -semantics=stage -mode=enum -argument=$additional -oracle=$ORACLE -depthArg=-1
	else
...
# 	fi
	...
	elif [ "$problem" = "SE-PR" ] || [ "$problem" = "SE-SST" ] || [ "$problem" = "SE-STG" ];
	then
		echo $output | sed 's/ACCEPT.*//' | sed 's/,\]/\]/g' 
