...
sys.path.insert(0, "{run.root}/submodules/python/")
from compression import get_compressed_file
...
args.filename = os.path.realpath('{run.root}/{run.instance}')
...
def main(args):
  ...
  cf = get_compressed_file(args.filename)
  if cf is None:
    sys.stdout.write('%s ERROR %s\n' % ('*' * 40, '*' * 40))
    sys.stdout.write('%s ERROR %s\n' % ('*' * 40, '*' * 40))

    sys.stdout.write('%s %s %s %s' %(args.filename, 'is a', cf.mime_type, 'file'))
    exit(1)

  #extract tempfile
  with tempfile.NamedTemporaryFile(dir=tmp,delete=False) as temp_out:
    for line in cf.accessor:
      temp_out.write(line)
    temp_out.flush()

    cmd = '%s -M %s -W %s -w %s %s %s %s > %s 2>> %s' % (args.runsolver, args.memlimit, args.timelimit, args.watcher, args.solver, ''.join(args.solver_args), temp_out.name, args.stdout, args.stderr)
    ...
    if int(p_solver.returncode) == 0 and childreturncode in (10,20):
